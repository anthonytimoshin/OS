...
The shortest execution time was expected to be with an 8-thread configuration (one thread per core), because increasing the number of threads requires more time to create and distribute the work among them. Therefore, the overhead exceeds the benefit of parallelization. However, as we can see, the shortest execution time is achieved with a 32-thread configuration. Firstly, this is because the program is not 100% CPU-bound. Secondly, the educational and test sets are quite large, and parallelization offsets the overhead of thread creation. Also, with smaller data sets, the load is distributed more evenly. However, increasing the number of threads to 64 results in an increase in execution time, which is due to thread competition and even greater overhead.
